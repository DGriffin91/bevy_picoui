...
#import bevy_coordinate_systems::transformations as transformations
...
fn fragment(in: MeshVertexOutput) -> @location(0) vec4<f32> {
    ...
    let ndc_a = transformations::frag_coord_to_ndc(in.frag_coord);
    let ndc_b = transformations::position_world_to_ndc(in.world_position.xyz);
    let ndc_c = transformations::position_view_to_ndc(transformations::position_world_to_view(in.world_position.xyz));
    let compare_ndc_1 = f32(distance(ndc_a, ndc_b) < 0.000001);
    let compare_ndc_2 = f32(distance(ndc_a, ndc_c) < 0.000001);

    let p = transformations::direction_world_to_view(transformations::direction_view_to_world(in.world_position.xyz));
    let compare_pos = f32(distance(p, in.world_position.xyz) < 0.0000001);

    let world_a = transformations::position_view_to_world(transformations::position_ndc_to_view(ndc_a));
    let world_b = transformations::position_ndc_to_world(ndc_a);
    ...
    let world_to_clip_pos = transformations::position_clip_to_world(transformations::position_world_to_clip(in.world_position.xyz));
    let compare_clip_to_world_pos = f32(distance(world_to_clip_pos, in.world_position.xyz) < 0.00001);

    let world_to_clip_dir = transformations::direction_clip_to_world(transformations::direction_world_to_clip(N));
    let compare_clip_to_world_dir = f32(distance(world_to_clip_dir, N) < 0.00000001);

    let a_dir = transformations::direction_view_to_world(transformations::direction_clip_to_view(transformations::direction_view_to_clip(transformations::direction_world_to_view(N))));
